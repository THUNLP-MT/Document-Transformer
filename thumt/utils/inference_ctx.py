...
def create_inference_graph(model_fns, features, params):
    ...
    # Expand the inputs in to the beam size
    # [batch, length] => [batch, beam_size, length]
    features["context"] = tf.expand_dims(features["context"], 1)
    features["context"] = tf.tile(features["context"], [1, beam_size, 1])
    shape = tf.shape(features["context"])

    # [batch, beam_size, length] => [batch * beam_size, length]
    features["context"] = tf.reshape(features["context"],
                                    [shape[0] * shape[1], shape[2]])

    # For context sequence length
    features["context_length"] = tf.expand_dims(features["context_length"], 1)
    features["context_length"] = tf.tile(features["context_length"],
                                        [1, beam_size])
    shape = tf.shape(features["context_length"])

    # [batch, beam_size, length] => [batch * beam_size, length]
    features["context_length"] = tf.reshape(features["context_length"],
                                           [shape[0] * shape[1]])
